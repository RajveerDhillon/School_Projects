...
volatile uint32_t *add_control = NULL;
volatile uint32_t *add_data = NULL;
int success, total, success2, total2;
...
void inputmult(uint32_t a, uint32_t b){
...
void reset_add(){
	alt_write_word(add_control+1, 0x1); //assert reset
	while(!(alt_read_word(add_control+1) & 0x1));
	
	printf("Reset done. Deasserting signal\n");
	while((alt_read_word(add_control+1) & 0x1));//deassert
}

void inputadd(uint32_t a, uint32_t b){
	
	alt_write_word(add_data, a); //write to unit
	alt_write_word(add_data+1, b); //write to unit

	//start conversion
	alt_write_word(add_control, 0x00000001);

	//double check that start was asserted
	while(!(alt_read_word(add_control) & 0x1));
...
void outputadd(){
	...
	//wait for done	
	printf("waiting for done\n");
	while(!(alt_read_word(add_control+2) & 0x1));

	printf("conversion done\n");
	word = alt_read_word(add_data+0);
	op1 = alt_read_word(add_data+1);
	op2 = alt_read_word(add_data+2);
	printf("0x%08x * 0x%08x = 0x%08x. [Expected] 0x%08x\n",  op1, op2, word, (op1+op2));
	if(word == (op1+op2)){
		printf("[SUCCESSFUL]\n");
		success2++;
...
int main(int argc, char **argv){
	...
	add_control = virtual_base + ((uint32_t)( ADD_CONTROL_0_BASE) );
	add_data = virtual_base + ((uint32_t)(ADD_DATA_0_BASE));
	
	printf("------>Finished initializing HPS/FPGA system<-------\n");

	for(i = 1; i < 31; i++){
	printf("----------- Multiplier Iteration %d -------------\n", i);
		reset_mult();
		inputmult((uint32_t)i, (uint32_t)i+1);
		outputmult();
	}	
	printf("[Multiplier PASSED] %d/%d\n", success, total);

	for(i = 1; i < 31; i++){
	printf("------------- Adder Iteration %d ---------------\n", i);
		reset_add();
		inputadd((uint32_t)i, (uint32_t)i+1);
		outputadd();
	}
	printf("[Adder PASSED] %d/%d\n", success, total);
